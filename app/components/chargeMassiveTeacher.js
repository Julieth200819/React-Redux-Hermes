...
const TeacherTableMassive = ({ title }) => {
        ...
        const rows = json.slice(1).map(row => {
          ...
          return {
            data: {
              ...
              email: String(row[5]),
              stateT: String(row[6]),
              date_birth: String(row[7] || ''),
              blood_type: String(row[8] || ''),
  ...
  const handleAddUsers = async () => {
    try {
      const response = await instructorService.postMassiveTeachers(combinedData);
      notifySuccess("Instructores agregados con éxito")
...
export default TeacherTableMassive;
